...
import { ALL_LINKS_QUERY } from '../graphql/queries'
import { NEW_LINKS_SUBSCRIPTION, VOTE_SUBSCRIPTION } from '../graphql/subscriptions'
      ...
      <div>
        <div>
          {linksToRender.map((link, index) => (
            <Link key={link.id} index={page ? (page - 1) * LINKS_PER_PAGE + index : index}
                  updateStoreAfterVote={this._updateCacheAfterVote} link={link} />
          ))}
        </div>
        {isNewPage &&
